...
async function main() {
  ...
  const insertedRoles = await db.insert(schema.role).values([
  ...
  const insertedPermissions = await db.insert(schema.permission).values([
    ...
    .onConflictDoUpdate({
      ...
      set: { description: schema.permission.description, updatedAt: new Date() }
    })
